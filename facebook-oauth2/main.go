...
func GetAccessToken(client_id string, code string, secret string, callbackUri string) AccessToken {
	...
	response, err := http.Get("https://graph.facebook.com/oauth/access_token?client_id=" +
		...
		callbackUri + "&client_secret=" + secret +
		"&code=" + code)
...
func Home(w http.ResponseWriter, r *http.Request) {
	...
	fbConfig := &oauth2.Config{
		...
		ClientID:     "664554397314079", // change this to yours
		ClientSecret: "85d0d5869f6a960ceef1ace6f77e817f",
	...
	_, _ = w.Write([]byte("<html><title>Golang Login Facebook Example</title> <body> <a href='" + url + "'><button>Login with Facebook!</button> </a> </body></html>"))
}
...
func FBLogin(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write([]byte(fmt.Sprintf("Username %s ID is %s and birthday is %s and email is %s<br>", fbusername, id, bday, email)))

	img := "https://graph.facebook.com/" + id + "/picture?width=180&height=180"

	_, _ = w.Write([]byte("Photo is located at " + img + "<br>"))
	// see https://www.socketloop.com/tutorials/golang-download-file-example on how to save FB file to disk

	_, _ = w.Write([]byte("<img src='" + img + "'>"))
}
